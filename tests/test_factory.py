...
class FactoryTest(TestCase):
    ...
    def test_fields(self):
        margherita = factory.make("tests.Pizza", fields={"name": "four cheeses"})
    ...
    def test_sequence_callable(self):
        margheritas = factory.make(
            "tests.Pizza", fields={"baked_on": timezone.now}, quantity=10
        )
        self.assertEqual(len(margheritas), 10)
    ...
    def test_password(self):
        user = factory.make(
            "auth.User", fields={"username": "username", "password": "password"}
        )
        self.assertTrue(user.check_password("password"))
    ...
    def test_g_m(self):
        ...
        chef_gusteau, created = factory.g_m(
            "tests.Chef", lookup={"last_name": "Gusteau"}
        )(first_name="Remi")
        self.assertEqual(Chef.objects.count(), 1)
        self.assertEqual(already_there, chef_gusteau)
        self.assertFalse(created)
        ...
        chef_linguini, created = factory.g_m(
            "tests.Chef", lookup={"last_name": "Linguini"}
        )(first_name="Alfredo")
    ...
    def test_rel(self):
        pizzas = factory.make("tests.Pizza", quantity=5, fields={"chef": rels.SELECT})
    ...
    def test_format(self):
        chef = factory.make(Chef, fields={"last_name": "{Linguini}"})
